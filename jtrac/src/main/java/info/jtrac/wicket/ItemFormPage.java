	...
	private class ItemForm extends Form {
		...
		public ItemForm(String id, final Item item) {
			...
			if (editMode) {
				...
				List<User> assignable = space.filterUsersAbleToTransitionFrom(userSpaceRoles, State.OPEN);
		...
		protected void onSubmit() {
			...
			if (editMode) {
				item = getJtrac().updateItem(item, user);
			} else {
				item.setLoggedBy(user);
				item.setStatus(State.OPEN);
				try {
					setModelObject(getJtrac().storeItem(item, getUploadedFile(fileUploadField)));
				} catch (IOException e) {
