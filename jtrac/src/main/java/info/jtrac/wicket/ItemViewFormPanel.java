...
import static info.jtrac.wicket.BasePage.*;
...
public class ItemViewFormPanel extends BasePanel {
	...
	private class ItemViewForm extends Form {
		...
		public ItemViewForm(String id, final Item item) {
			...
			statusChoice.add(new AjaxFormComponentUpdatingBehavior("onChange") {
				...
				protected void onUpdate(AjaxRequestTarget target) {
					...
					if (selectedStatus == null) {
						...
						List<User> assignable = space.filterUsersAbleToTransitionFrom(userSpaceRoles, selectedStatus);
			...
			add(statusChoice);
			// notify list =====================================================
			List<ItemUser> choices = Space.convertToItemUserList(userSpaceRoles);
		...
		protected void onSubmit() {
			...
			try {
				getJtrac().storeHistoryForItem(itemId, history, getUploadedFile(fileUploadField));
			} catch (IOException e) {
