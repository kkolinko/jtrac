...
/**
 * Standard User entity with attributes such as name, password etc. The parent
 * relationship is used for easy grouping of users and flexible inheritance of
 * permission schemes TODO. The user type determines if this is a normal user or
 * a user group. Only user groups can have child references.
 *
 * We also tie in to the Acegi security framework and implement the Acegi
 * UserDetails interface so that Acegi can take care of Authentication and
 * Authorization
 */
public class User implements UserDetails, Serializable, Comparable<User> {
	...
	/**
	 * used because there is a rare chance that after a principal "refresh" like
	 * after editing spaces for self, on saving profile for self, the "fake" roles
	 * like "ROLE_GUEST" will get saved to the database this method has to be
	 * called before saving a user object
	 */
	...
	/**
	 * when the passed space is null this has a special significance it will
	 * return roles that are 'global'
	 */
	...
	/**
	 * this returns 'valid' spaceRoles, where space not null and role not
	 * ROLE_ADMIN also sort by Space name for showing on the dashboard TODO
	 * multiple roles per space
	 */
	...
	/**
	 * returns a sorted map of spaces for this user where the value is a List of
	 * role keys, useful for UI display of this users allocated spaces and roles
	 */
	 ...
	 */
	public Map<Long, String> getSpaceNamesMap() {
		Map<Long, String> map = new HashMap<Long, String>(userSpaceRoles.size());
		for (UserSpaceRole usr : userSpaceRoles) {
			Space space = usr.getSpace();
			if (space != null) {
				map.put(space.getId(), space.getName());
			}
		}
		return map;
	}
